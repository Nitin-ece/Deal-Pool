      ...
      <div className="flex flex-col sm:flex-row gap-2.5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white rounded-xl text-xs sm:text-sm text-[var(--ink)] border border-[var(--line)] focus:outline-none focus:ring-2 focus:ring-[var(--signal)] transition-all placeholder:text-gray-400"
          />
          {searchQuery && (
            <button
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="px-3.5 py-2.5 bg-white rounded-xl text-xs font-semibold text-gray-700 border border-[var(--line)] focus:outline-none focus:ring-2 focus:ring-[var(--signal)] transition-all cursor-pointer"
          >
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium whitespace-nowrap transition-colors cursor-pointer ${
                isActive
                  ? "bg-[var(--ink)] text-white font-semibold shadow-xs"
                  : "bg-white text-gray-600 hover:bg-gray-50 border border-[var(--line)]"
              }`}
            >
              {cat.icon}
              <span>{cat.label}</span>
